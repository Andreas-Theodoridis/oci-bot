...
CREATE TABLE CHATBOT_REASONING_LOG 
    ( 
     ID               NUMBER GENERATED BY DEFAULT AS IDENTITY 
        ( START WITH 1 CACHE 20 ) , 
     CHAT_ID          NUMBER , 
...
ALTER TABLE CHATBOT_REASONING_LOG 
    ADD PRIMARY KEY ( ID ) 
    USING INDEX LOGGING ;

create or replace PROCEDURE OCI_GENAI_BOT.CHATBOT_PROC (
    ...
    p_row_id          OUT chatbot_log.id%TYPE,
    ...
    v_response           CLOB;
    v_aimessage          CLOB;
    v_log_row_id         chatbot_log.id%TYPE;
...
BEGIN
    ...
    EXCEPTION
        WHEN OTHERS THEN
            apex_debug.message('������ Failed loading prompt components: %s', sqlerrm);
    ...
    LOOP
        ...
        BEGIN
            ...
            IF l_has_data THEN
                ...
                EXCEPTION
                    WHEN OTHERS THEN
                        IF dbms_sql.is_open(l_cursor) THEN
                            dbms_sql.close_cursor(l_cursor);
                        END IF;
                        apex_debug.message('������ Failed to build AIMESSAGE JSON: %s', sqlerrm);
                        p_final_aimessage := '[]';
                END;
                ...
                BEGIN
                    ...
                    IF l_resp_obj.has('chatResponse') THEN
                        ...
                        BEGIN
                            ...
                            ELSE
                                apex_debug.message('������ No summary text found to store.');
                            END IF;
                        END;
                    END IF;
                ...
                EXIT;
            ELSE
                apex_debug.message('������ SQL produced no results.');
                v_last_failed := 'Y';
            END IF;
        ...
        THEN
            apex_debug.message('������ Triggering GenAI rephrasing');
            ...
            IF l_resp_obj.has('chatResponse') THEN
                ...
                BEGIN
                    ...
                    INSERT INTO CHATBOT_REASONING_LOG (
                        chat_id,
                        log_id,
                        input_message,
                        rephrased_output,
